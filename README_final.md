...
## AlphaFold 3 model outputs

`folds_2026_06_22_14_59.zip` contains the complete AlphaFold 3 Server outputs for all 21 modelled sites (7 Tier 1, 7 Tier 2, 7 no-response). Each job folder holds the job request (including the model seed), the five ranked models, and the corresponding confidence files. Unzip it alongside `analyse_af3_fig7.py` before running that script.
...
The AlphaFold 3 model outputs are included in this repository. Other input datasets are not.
...
The Phase 1 sequencing data and the processed enrichment tables are available from the author on request.
